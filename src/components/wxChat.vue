...
<template>
  <div class="wxchat-container">
    <div class="window"
      ...
      <div class="title"
           v-if="dataArray && dataArray.length>0 && false">
        <p v-text="contactNickname"></p>
      </div>
      <!-- main -->
      <ScrollLoader :minHeight="minHeight"
        ...
        <div class="message">
          <ul style="padding-left: 0px;">
            <li v-for="message in dataArray"
                :key="message.id"
                :class="message.direction==2?'an-move-right':'an-move-left'">
              <p class="time"><span v-text="formatTime(message.ctime)"></span>
                <span v-text="message.re_mark"></span></p>
              <p class="time system"
                 v-if="message.type==10000"><span v-html="message.content"></span></p>
              <div :class="'main' + (message.direction==2?' self':'')"
                   v-else>
<!--                <img class="avatar"-->
<!--                     width="45"-->
<!--                     height="45"-->
<!--                     :src="message.create_user_avatar">-->
                <avatar class="avatar" :username="message.create_user_name"  style="width: 45px;height:45px"></avatar>
